...
HRESULT QueryCapsForSelectedDevice(IBaseFilter* pGameCaptureFilter, VIDEO_CAPTURE_FILTER_DEVICE_CAPS* pCaps)
{
	...
	if (SUCCEEDED(hr))
	{
		...
		{
			ElgatoGameCapture::VIDEO_CAPTURE_FILTER_DEVICE_CAPS deviceCaps;
...
int videoInput::start(int deviceID, videoDevice *VD) {
	...
	if (strncmp(deviceNames[deviceID], "Elgato", 6) == 0) {
		// Add "Elgato Game Capture HD" filter
		hr = CoCreateInstance(CLSID_ElgatoVideoCaptureFilter, NULL, CLSCTX_INPROC, IID_IBaseFilter, (void **)&VD->pVideoInputFilter);
		_ASSERT(SUCCEEDED(hr));

		hr = VD->pGraph->AddFilter(VD->pVideoInputFilter, L"Elgato Game Capture HD");
		_ASSERT(SUCCEEDED(hr));

		/*
		ElgatoGameCapture::VIDEO_CAPTURE_FILTER_DEVICE_CAPS caps;
		hr = QueryCapsForSelectedDevice(VD->pVideoInputFilter, &caps);
		_ASSERT(SUCCEEDED(hr));

		CComPtr<IPin> pMpegTsOutput;
		HRESULT hrTmp = VD->pCaptureGraph->FindPin(VD->pVideoInputFilter, PINDIR_OUTPUT, NULL, &MEDIATYPE_Stream, TRUE, 0, &pMpegTsOutput);
		bool hasMpegTsPin = SUCCEEDED(hrTmp); // Elgato note: older versions of the capture filter don't have an MPEG-TS pin

		if (caps.hardwareDeliversCompressedVideoData)
		{
		printf("HAS MPEG_TS PIN\n");
		}

		if (caps.hardwareDeliversRawVideoData)
		{
		printf("HAS RAW VIDEO\n");
		std::wcout << caps.deviceName << std::endl;
	...
	if (VD->pAmMediaType->formattype == FORMAT_VideoInfo)
	{
		// Check the buffer size.
		if (VD->pAmMediaType->cbFormat >= sizeof(VIDEOINFOHEADER))
		{
			VIDEOINFOHEADER *pVih = reinterpret_cast<VIDEOINFOHEADER*>(VD->pAmMediaType->pbFormat);
			/* Access VIDEOINFOHEADER members through pVih. */
			currentWidth = HEADER(pVih)->biWidth;
			currentHeight = HEADER(pVih)->biHeight;
			avgTimePerFrame = pVih->AvgTimePerFrame;
		}
	}
	else if (VD->pAmMediaType->formattype == FORMAT_VideoInfo2) {
		
		// Check the buffer size.
		if (VD->pAmMediaType->cbFormat >= sizeof(VIDEOINFOHEADER2))
		{
			VIDEOINFOHEADER2 *pVih = reinterpret_cast<VIDEOINFOHEADER2*>(VD->pAmMediaType->pbFormat);
			/* Access VIDEOINFOHEADER2 members through pVih. */
			currentWidth = pVih->bmiHeader.biWidth;
			currentHeight = pVih->bmiHeader.biHeight;
			avgTimePerFrame = pVih->AvgTimePerFrame;
		}
	}
	...
	if(customSize == false || foundSize == false){
		if( VD->requestedFrameTime != -1 ){
			avgTimePerFrame = VD->requestedFrameTime;
			hr = VD->streamConf->SetFormat(VD->pAmMediaType);
		}
